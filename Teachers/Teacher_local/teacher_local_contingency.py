...
class Contingency_Sentence_Words_Embeddings(nn.Module):
    # input: input_ids, token_type_ids, attention_mask
    # output: batch_size * number of tokens * 768
    def __init__(self):
        super(Contingency_Sentence_Words_Embeddings, self).__init__()
...
class Contingency_Model(nn.Module):
    def __init__(self):
        super(Contingency_Model, self).__init__()

        self.sentence_words_embeddings = Contingency_Sentence_Words_Embeddings()
    ...
    def forward(self, arg1_input_ids, arg1_token_type_ids, arg1_attention_mask, arg2_input_ids, arg2_token_type_ids, arg2_attention_mask, comparison_label, contingency_label):
        ...
        supervised_loss = self.supervised_criterion(relation_score, contingency_label)
...
contingency_model = Contingency_Model()
contingency_model.cuda()


param_all = list(contingency_model.named_parameters())
...
# contingency relation classifier training

best_macro_F = 0
best_contingency_F = 0
    ...
    for i in range(len(train_dataloader)):
        ...
        if i % 100 == 0 and not i == 0:
            ...
            for j in range(len(dev_dataloader)):
                ...
                with torch.no_grad():
                    relation_score, supervised_loss = contingency_model(arg1_input_ids, arg1_token_type_ids, arg1_attention_mask,
                ...
                true_label = contingency_label.view(relation_score.shape[0], 1)  # true label is contingency label
            ...
            contingency_F = precision_recall_fscore_support(true_label_onetest, decision_supervised_onetest, average='binary')[2]

            if macro_F > best_macro_F:
                torch.save(contingency_model.state_dict(), "./saved_models/contingency_best_macro_F.ckpt")
                best_macro_F = macro_F

            if contingency_F > best_contingency_F:
                torch.save(contingency_model.state_dict(), "./saved_models/contingency_best_contingency_F.ckpt")
                best_contingency_F = contingency_F

            print("MACRO: ", precision_recall_fscore_support(true_label_onetest, decision_supervised_onetest, average='macro'))
            print("CONTINGENCY CLASS: ", precision_recall_fscore_support(true_label_onetest, decision_supervised_onetest, average='binary'))

        contingency_model.train()
        ...
        torch.nn.utils.clip_grad_norm_(contingency_model.parameters(), 1.0)
        optimizer.step()
        scheduler.step()
...
contingency_model.load_state_dict(torch.load('./saved_models/contingency_best_macro_F.ckpt', map_location=device))
...
# best contingency class model

contingency_model = Contingency_Model()
contingency_model.cuda()

contingency_model.load_state_dict(torch.load('./saved_models/contingency_best_contingency_F.ckpt', map_location=device))
...
print("Best contingency class F model")
print("MACRO: ", precision_recall_fscore_support(true_label_onetest, decision_supervised_onetest, average='macro'))
print("CONTINGENCY CLASS: ", precision_recall_fscore_support(true_label_onetest, decision_supervised_onetest, average='binary'))
...
def write_comp_prob(article_dir):
    ...
    out_file_name = './BASIL_cont_probability/' + article_dir[8:-5] + '.txt'
...
def write_comp_prob(article_dir):
    ...
    out_file_name = './BiasedSents_cont_probability/' + article_dir[14:-5] + '.txt'
